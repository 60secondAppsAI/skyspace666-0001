        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPilots = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPilots">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Pilot</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PilotId" type="text" placeholder="Enter PilotId" v-model="pilotToAdd.pilotId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="pilotToAdd.name"></base-input>
  <base-input label="LicenseNumber" type="text" placeholder="Enter LicenseNumber" v-model="pilotToAdd.licenseNumber"></base-input>
  <base-input label="YearsExperience" type="text" placeholder="Enter YearsExperience" v-model="pilotToAdd.yearsExperience"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="pilots" :row-key="record => record.PilotId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PilotPictureView :pilots="pilots" />
          </div>
...
import PilotPictureView from './PilotPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'pilotId',
          visible: true,
          scopedSlots: { customRender: 'pilotId' },
          sorter: true
          //sorter: (a, b) => a.pilotId - b.pilotId,
          //sorter: (a, b) => a.pilotId.localeCompare(b.pilotId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
		...
		dataIndex: 'yearsExperience',
          visible: true,
          scopedSlots: { customRender: 'yearsExperience' },
          sorter: true
          //sorter: (a, b) => a.yearsExperience - b.yearsExperience,
          //sorter: (a, b) => a.yearsExperience.localeCompare(b.yearsExperience),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} pilots`,
      },
      ...
      sortBy: 'pilotId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPilotsTable() {
      ...
      for (let i = 0; i < this.pilots.length; i++) {
        pilotsTableData.push({
          id: i,
          pilotId: this.pilots[i].pilotId,
          year: this.pilots[i].year,
          date: this.pilots[i].date,
          competitionId: this.pilots[i].competitionId,
          pilotId: this.pilots[i].pilotId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-pilots',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPilotsChanged', this.searchQuery);
		//this.renderPilotsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPilots = false;

      const currentDate = new Date().getTime();
      this.pilotToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.pilotToAdd);
      console.log(jsonData);
      
      const res = await PilotService.addPilot(jsonData);
