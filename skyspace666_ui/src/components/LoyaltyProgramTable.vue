        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalLoyaltyPrograms = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLoyaltyPrograms">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add LoyaltyProgram</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LoyaltyProgramId" type="text" placeholder="Enter LoyaltyProgramId" v-model="loyaltyProgramToAdd.loyaltyProgramId"></base-input>
  <base-input label="ProgramName" type="text" placeholder="Enter ProgramName" v-model="loyaltyProgramToAdd.programName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="loyaltyPrograms" :row-key="record => record.LoyaltyProgramId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LoyaltyProgramPictureView :loyaltyPrograms="loyaltyPrograms" />
          </div>
...
import LoyaltyProgramPictureView from './LoyaltyProgramPictureView.vue';


const loyaltyProgramsColumns = [
  "loyaltyProgramId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    LoyaltyProgramPictureView
  },

  data() {
    return {
      modalLoyaltyPrograms: false,
        isTableView: true,

      columns: [
        {
          title: 'Loyalty Program Id',
		dataIndex: 'loyaltyProgramId',
          visible: true,
          scopedSlots: { customRender: 'loyaltyProgramId' },
          sorter: true
          //sorter: (a, b) => a.loyaltyProgramId - b.loyaltyProgramId,
          //sorter: (a, b) => a.loyaltyProgramId.localeCompare(b.loyaltyProgramId),
        },
        {
          title: 'Program Name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} loyaltyPrograms`,
      },
      ...
      sortBy: 'loyaltyProgramId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLoyaltyProgramsTable() {
      ...
      let loyaltyProgramsTableData = [];
      for (let i = 0; i < this.loyaltyPrograms.length; i++) {
        loyaltyProgramsTableData.push({
          id: i,
          loyaltyProgramId: this.loyaltyPrograms[i].loyaltyProgramId,
          year: this.loyaltyPrograms[i].year,
          date: this.loyaltyPrograms[i].date,
          competitionId: this.loyaltyPrograms[i].competitionId,
          loyaltyProgramId: this.loyaltyPrograms[i].loyaltyProgramId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-loyalty-programs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLoyaltyProgramsChanged', this.searchQuery);
		//this.renderLoyaltyProgramsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLoyaltyPrograms = false;

      const currentDate = new Date().getTime();
      this.loyaltyProgramToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.loyaltyProgramToAdd);
      console.log(jsonData);
      
      const res = await LoyaltyProgramService.addLoyaltyProgram(jsonData);
