        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">In Flight Meals</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalInFlightMeals = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInFlightMeals">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add InFlightMeal</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="InFlightMealId" type="text" placeholder="Enter InFlightMealId" v-model="inFlightMealToAdd.inFlightMealId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="inFlightMeals" :row-key="record => record.InFlightMealId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InFlightMealPictureView :inFlightMeals="inFlightMeals" />
          </div>
...
import InFlightMealPictureView from './InFlightMealPictureView.vue';


const inFlightMealsColumns = [
  "inFlightMealId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    InFlightMealPictureView
  },

  data() {
    return {
      modalInFlightMeals: false,
        isTableView: true,

      columns: [
        {
          title: 'In Flight Meal Id',
		dataIndex: 'inFlightMealId',
          visible: true,
          scopedSlots: { customRender: 'inFlightMealId' },
          sorter: true
          //sorter: (a, b) => a.inFlightMealId - b.inFlightMealId,
          //sorter: (a, b) => a.inFlightMealId.localeCompare(b.inFlightMealId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} inFlightMeals`,
      },
      ...
      inFlightMealsTable: {
        columns: [...inFlightMealsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'inFlightMealId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInFlightMealsTable() {
      ...
      let inFlightMealsTableData = [];
      for (let i = 0; i < this.inFlightMeals.length; i++) {
        inFlightMealsTableData.push({
          id: i,
          inFlightMealId: this.inFlightMeals[i].inFlightMealId,
          year: this.inFlightMeals[i].year,
          date: this.inFlightMeals[i].date,
          competitionId: this.inFlightMeals[i].competitionId,
          inFlightMealId: this.inFlightMeals[i].inFlightMealId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-in-flight-meals',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInFlightMealsChanged', this.searchQuery);
		//this.renderInFlightMealsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalInFlightMeals = false;

      const currentDate = new Date().getTime();
      this.inFlightMealToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.inFlightMealToAdd);
      console.log(jsonData);
      
      const res = await InFlightMealService.addInFlightMeal(jsonData);
