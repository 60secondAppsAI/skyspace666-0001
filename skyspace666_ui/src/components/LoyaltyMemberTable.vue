        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalLoyaltyMembers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLoyaltyMembers">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add LoyaltyMember</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LoyaltyMemberId" type="text" placeholder="Enter LoyaltyMemberId" v-model="loyaltyMemberToAdd.loyaltyMemberId"></base-input>
  <base-input label="Points" type="text" placeholder="Enter Points" v-model="loyaltyMemberToAdd.points"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="loyaltyMembers" :row-key="record => record.LoyaltyMemberId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LoyaltyMemberPictureView :loyaltyMembers="loyaltyMembers" />
          </div>
...
import LoyaltyMemberPictureView from './LoyaltyMemberPictureView.vue';


const loyaltyMembersColumns = [
  "loyaltyMemberId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    LoyaltyMemberPictureView
  },

  data() {
    return {
      modalLoyaltyMembers: false,
        isTableView: true,

      columns: [
        {
          title: 'Loyalty Member Id',
		dataIndex: 'loyaltyMemberId',
          visible: true,
          scopedSlots: { customRender: 'loyaltyMemberId' },
          sorter: true
          //sorter: (a, b) => a.loyaltyMemberId - b.loyaltyMemberId,
          //sorter: (a, b) => a.loyaltyMemberId.localeCompare(b.loyaltyMemberId),
        },
        {
          title: 'Points',
		dataIndex: 'points',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} loyaltyMembers`,
      },
      ...
      sortBy: 'loyaltyMemberId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLoyaltyMembersTable() {
      ...
      let loyaltyMembersTableData = [];
      for (let i = 0; i < this.loyaltyMembers.length; i++) {
        loyaltyMembersTableData.push({
          id: i,
          loyaltyMemberId: this.loyaltyMembers[i].loyaltyMemberId,
          year: this.loyaltyMembers[i].year,
          date: this.loyaltyMembers[i].date,
          competitionId: this.loyaltyMembers[i].competitionId,
          loyaltyMemberId: this.loyaltyMembers[i].loyaltyMemberId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-loyalty-members',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLoyaltyMembersChanged', this.searchQuery);
		//this.renderLoyaltyMembersTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLoyaltyMembers = false;

      const currentDate = new Date().getTime();
      this.loyaltyMemberToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.loyaltyMemberToAdd);
      console.log(jsonData);
      
      const res = await LoyaltyMemberService.addLoyaltyMember(jsonData);
