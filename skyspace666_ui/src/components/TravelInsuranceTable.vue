...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Travel Insurances</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalTravelInsurances = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalTravelInsurances">
                <template slot="header">
  ...
  <base-input label="TravelInsuranceId" type="text" placeholder="Enter TravelInsuranceId" v-model="travelInsuranceToAdd.travelInsuranceId"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="travelInsurances" :row-key="record => record.TravelInsuranceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <TravelInsurancePictureView :travelInsurances="travelInsurances" />
          </div>
...
import TravelInsurancePictureView from './TravelInsurancePictureView.vue';


const travelInsurancesColumns = [
  "travelInsuranceId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    TravelInsurancePictureView
  },

  data() {
    return {
      modalTravelInsurances: false,
        isTableView: true,

      columns: [
        {
          title: 'Travel Insurance Id',
		dataIndex: 'travelInsuranceId',
          visible: true,
          scopedSlots: { customRender: 'travelInsuranceId' },
          sorter: true
          //sorter: (a, b) => a.travelInsuranceId - b.travelInsuranceId,
          //sorter: (a, b) => a.travelInsuranceId.localeCompare(b.travelInsuranceId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} travelInsurances`,
      },

      travelInsurances: [],
      travelInsuranceToAdd: {},

      travelInsurancesTable: {
        columns: [...travelInsurancesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'travelInsuranceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderTravelInsurancesTable() {
      ...
      let travelInsurancesTableData = [];
      for (let i = 0; i < this.travelInsurances.length; i++) {
        travelInsurancesTableData.push({
          id: i,
          travelInsuranceId: this.travelInsurances[i].travelInsuranceId,
          year: this.travelInsurances[i].year,
          date: this.travelInsurances[i].date,
          competitionId: this.travelInsurances[i].competitionId,
          travelInsuranceId: this.travelInsurances[i].travelInsuranceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-travel-insurances',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForTravelInsurancesChanged', this.searchQuery);
		//this.renderTravelInsurancesTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.travelInsuranceToAdd.created = currentDate;
