        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalInsurances = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInsurances">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Insurance</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="InsuranceId" type="text" placeholder="Enter InsuranceId" v-model="insuranceToAdd.insuranceId"></base-input>
  <base-input label="Provider" type="text" placeholder="Enter Provider" v-model="insuranceToAdd.provider"></base-input>
  <base-input label="CoverageAmount" type="text" placeholder="Enter CoverageAmount" v-model="insuranceToAdd.coverageAmount"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="insurances" :row-key="record => record.InsuranceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InsurancePictureView :insurances="insurances" />
          </div>
...
import InsurancePictureView from './InsurancePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'insuranceId',
          visible: true,
          scopedSlots: { customRender: 'insuranceId' },
          sorter: true
          //sorter: (a, b) => a.insuranceId - b.insuranceId,
          //sorter: (a, b) => a.insuranceId.localeCompare(b.insuranceId),
        },
        {
          title: 'Provider',
		dataIndex: 'provider',
		...
		dataIndex: 'coverageAmount',
          visible: true,
          scopedSlots: { customRender: 'coverageAmount' },
          sorter: true
          //sorter: (a, b) => a.coverageAmount - b.coverageAmount,
          //sorter: (a, b) => a.coverageAmount.localeCompare(b.coverageAmount),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'insuranceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInsurancesTable() {
      ...
      let insurancesTableData = [];
      for (let i = 0; i < this.insurances.length; i++) {
        insurancesTableData.push({
          id: i,
          insuranceId: this.insurances[i].insuranceId,
          year: this.insurances[i].year,
          date: this.insurances[i].date,
          competitionId: this.insurances[i].competitionId,
          insuranceId: this.insurances[i].insuranceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-insurances',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInsurancesChanged', this.searchQuery);
		//this.renderInsurancesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalInsurances = false;

      const currentDate = new Date().getTime();
      this.insuranceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.insuranceToAdd);
      console.log(jsonData);
      
      const res = await InsuranceService.addInsurance(jsonData);
