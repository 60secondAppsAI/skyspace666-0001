					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateTerminal()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Terminals = true">Add</base-button>
								              <modal :show.sync="modal2Terminals">
								  ...
								  <base-input label="Name" type="text" placeholder="Enter Name" v-model="terminalToAdd.name"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Terminal Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="TerminalId" type="text" placeholder="Enter TerminalId" v-model="terminalDetails.terminalId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Name" type="text" placeholder="Enter Name" v-model="terminalDetails.name"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateTerminal() {
      ...
      const jsonData = JSON.stringify(this.terminalDetails);
      const res = await TerminalService.update(jsonData);
...
//        this.getAllTerminals();
    ...
    async getTerminalDetails() {
      try {
			let response = await TerminalService.get(this.terminalId);
